...
class AngleRegressionModel(nn.Module):
    ...
    def _batch_to_feature(self, batch: dict):
        ...
        #mask
        masked_aa = torch.cat([batch['rec_aa'], torch.full_like(batch['pep_aa'], 20)], dim=1)  # (B,L)
        ...
        res_feature = torch.cat([
            one_hot(masked_aa, num_classes=21).float(),
    ...
    def forward(self, rot, pos, res_feature, pair_feature, mask, max_rec_length=224, max_pep_length=32):
        ...
        logits = self.classification_layer(pep_embedding).reshape(B, L2, self.NUM_AA_TYPES)  # (B, L2, 2, 2)
        assert logits.shape == (B, L2, self.NUM_AA_TYPES)
        return logits

    def get_loss(self, batch):
        features = self._batch_to_feature(batch)
        pred_logits = self.forward(*features)  # (B, L2, AA_TYPES)
        
        # load label
        if self.predict_target == 'this':
            label_types = batch['pep_aa']  # (B, L2)
            label_types_mask = (batch['pep_aa'] != 20)
        elif self.predict_target == 'next':
            label_types = batch['label_types'][:, :, 1]
            label_types_mask = batch['label_types_mask'][:, :, 1]
        elif self.predict_target == 'prev':
            label_types = batch['label_types'][:, :, 0]
            label_types_mask = batch['label_types_mask'][:, :, 0]
            
        B, L, _ = pred_logits.shape

        assert pred_logits.shape == (B, L, self.NUM_AA_TYPES)
        assert label_types.shape == (B, L)
        assert label_types_mask.shape == (B, L)
        
        if self.mask_type == 'single':
            # only use the first position
            pred_logits = pred_logits[:, :1]
            label_types = label_types[:, :1]
            label_types_mask = label_types_mask[:, :1]
        
        loss_clf = cross_entropy_with_mask(pred_logits, label_types, label_types_mask)
        accuracy = accuracy_with_mask(pred_logits, label_types, label_types_mask)
        
        def get_hist(logits, label, mask):
        ...
        hist_pred, hist_label = get_hist(pred_logits, label_types, label_types_mask)

        return {'loss_clf': loss_clf}, {
            'accuracy': accuracy,
...
if __name__ == '__main__':
    ...
    wandb.init(project='PepDesign', name=f'test_classification_{args.predict_target}_{args.mask_type}' + datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S'), config=config)

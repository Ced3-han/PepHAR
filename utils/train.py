...
def log_losses_wandb(out, it, tag, logger=BlackHole(), writer=BlackHole(), others={}):
    logstr = '[%s] Iter %05d' % (tag, it)
    logstr += ' | loss %.4f' % out['overall'].item()
    for k, v in out.items():
        if k == 'overall': continue
    ...
    for k, v in out.items():
        if k == 'overall':
            wandb.log({f'{tag}/loss': v}, commit=True)  # doesn't increase step
            # writer.add_scalar('%s/loss' % tag, v, it)
        else:
            wandb.log({f'{tag}/loss_{k}': v}, commit=False)  # doesn't increase step
            # writer.add_scalar('%s/loss_%s' % (tag, k), v, it)
    for k, v in others.items():
        wandb.log({f'{tag}/{k}': v}, commit=False)  # increase step
        # writer.add_scalar('%s/%s' % (tag, k), v, it)
    # writer.flush()
...
class ValidationLossTape(object):
    ...
    def log(self, it, logger=BlackHole(), writer=BlackHole(), tag='val', others={}):
        ...
        log_losses_wandb(avg, it, tag, logger, writer, others={**avg_others, **others})
        return avg['overall']
